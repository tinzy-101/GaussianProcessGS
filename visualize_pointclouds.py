...
def read_points3D_text(path):
    """handles both integer and float RGB values"""
    xyzs, rgbs, errors = [], [], []
...
def load_point_cloud_from_colmap(file_path):
    ...
    try:
        ...
        xyzs, rgbs, errors = read_points3D_text(file_path)
...
def visualize_sparse_only(scene_name, base_path="mipnerf360"):
    ...
    if sparse_pcd is None:
        print("Error: Could not load sparse point cloud")
        return
    
    # Create visualizer
    vis = o3d.visualization.Visualizer()
    vis.create_window(f"Sparse Point Cloud - {scene_name}", width=1600, height=800)
    
    # Add coordinate frame for reference
    coordinate_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=1.0)
    vis.add_geometry(coordinate_frame)
    
    # Add sparse point cloud with original RGB colors
    ...
    print(f"\nSparse point cloud: {len(sparse_pcd.points)} points with original RGB colors")
    print("\nControls:")
...
def visualize_densified_only(scene_name, base_path="mipnerf360"):
    ...
    if densified_pcd is None:
        print("Error: Could not load densified point cloud")
        return
    
    # Create visualizer
    vis = o3d.visualization.Visualizer()
    vis.create_window(f"Densified Point Cloud - {scene_name}", width=1600, height=800)
    
    # Add coordinate frame for reference
    coordinate_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=1.0)
    vis.add_geometry(coordinate_frame)
    
    # Add densified point cloud with original RGB colors
    vis.add_geometry(densified_pcd)
    
    # Set up view
    vis.get_render_option().point_size = 2.0
    vis.get_render_option().background_color = [0.1, 0.1, 0.1]  # Dark background
    
    print(f"\nDensified point cloud: {len(densified_pcd.points)} points with original RGB colors")
    print("\nControls:")
    print("- Mouse: Rotate view")
    print("- Mouse wheel: Zoom")
    print("- Shift + Mouse: Pan")
...
def main():
    ...
    if args.type == "sparse":
        visualize_sparse_only(args.scene, args.base_path)
    elif args.type == "densified":
        visualize_densified_only(args.scene, args.base_path)
