...
for image_name in images:
    ...
    class VariationalMultiTaskGPModel(gpytorch.models.ApproximateGP):
        def __init__(self, 
            ...
            #inducing_points = farthest_point_sampling(train_x, num_inducing).unsqueeze(0).repeat(rank, 1, 1)  # shape: (rank, m, d)
            inducing_points = farthest_point_sampling(train_x, num_inducing)
            self.inducing_points = inducing_points
            print(f"inducing pts shape:{inducing_points.shape}")
            print(f"inducing_points.size(-2):{inducing_points.size(-2)}")


            # CHANGE: hard coded var_dist 
            variational_distribution = gpytorch.variational.CholeskyVariationalDistribution(
                num_inducing_points=inducing_points.size(-2), # size(-2) should equal num_inducing 
                batch_shape=torch.Size([rank])
            )

            print("variational_distribution.shape:", variational_distribution.variational_mean.shape)
            ...
            self.covar_module = LCMKernel(base_kernels, 
                                          num_tasks=num_tasks, 
                                          rank=rank)
        ...
        def fit(self,
            ...
            print("INPUT TO MODEL:", train_x.shape)
            print("self.variational_strategy.inducing_points.shape:", self.variational_strategy.base_variational_strategy.inducing_points.shape)
            print("self.variational_strategy.variational_distribution.variational_mean.shape:", self.variational_strategy.base_variational_strategy._variational_distribution.variational_mean.shape)
                        
            self.train()
            self.likelihood.train()

            print(f"train_input:{train_x.shape}")
            print(f"train_output:{train_y.shape}")

            #dataset = TensorDataset(train_x, train_y)
            #dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)

            # CHANGE: hardcoded optimizer
            if reset_opt or self._optimizer is None: 
                # self._optimizer = torch.optim.Adagrad([ # optimize using only model and likelihood params
                #     {'params': self.parameters()},
                #     {'params': self.likelihood.parameters()},
                # ], lr=lr)
                self._optimizer = torch.optim.Adagrad(self.parameters(), lr=lr) # optimize using all available parameters
            else:
                for pg in self._optimizer.param_groups: # to preserve optimizer momentum, but update lr 
                    pg["lr"] = lr
            optimizer = self._optimizer 

            mll = gpytorch.mlls.VariationalELBO(self.likelihood, self, num_data=train_y.size(0))
            print(f"trainy size(0):{train_y.size(0)}")
    ...
    base_kernels = [MaternKernel(nu=NU),MaternKernel(nu=NU),MaternKernel(nu=NU)]

    model = VariationalMultiTaskGPModel(train_x=train_input.to(device),
                                        train_y=train_output.to(device),
    ...
    model.predict(test_x=test_input)
    ...
    true_output, mean_prediction = model.predict(test_input.to(device))
